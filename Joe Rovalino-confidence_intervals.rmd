...
JR Answer:
It ranges from low of 438 to high 1991. 

```{r}
summary(samp)
hist(samp)
```
The typical size is 1456. I got typical from looking at the sample and then running a summary for the sample

2.  Would you expect another student's distribution to be identical to yours? 
Would you expect it to be similar? Why or why not?

JR Answer:
The other students distribution will not be identical. It would be similar but not identical. This is because 60 are randomly being selected so the likelihood of picking the exact same 60 across a population of 2930 if very unlikely. 
...
JR Answer:
The conditions: Random, independent and data not skeweed
...
JR Answer:
Taking many samples and compute 95% conf. interval for each sample then about 95% of these conf. intervals will have population mean. 
...
JR answer;
Yes mine captures the true average size of houses. 
...
JR Answer: 
The proportion would be 95% of the confidence intervals to capture true population mean. 
...
JR Answer:
96% 
50 confidence intervals of size = 60. 2 of 50 intervals didn't fall in the mean -> so I got .96 
This is not exact to the 95 confidence interval. It is only supposed to be a range that is about 95% likely to contain the true value of the population.  

-   Pick a confidence level of your choosing, provided it is not 95%. What is 
    the appropriate critical value?
    
    I'll pick a 99% confidence level, using 1-(0.01/2) = .995
```{r}
cv = qnorm(.995)
cv
```
  Critical values are 2.575829
...
```
The proportion at 99% CI is 1.00